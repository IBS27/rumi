...
- Use Bun. Run `bun run typecheck`, `bun run lint`, and `bun test`. `bun run typecheck` and the `chat`, `pairing` and `agent-integration` tests need `convex/_generated`, so they fail until a deployment is configured; `bun run typecheck:shared` covers everything else and works with no deployment. Keep tests focused on behavior and use `bun:test`; check UI changes in the browser.
